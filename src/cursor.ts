...
export interface CursorCodec {
  encode(scope: string, state: string, position: unknown): string
  decode<T>(
    scope: string,
    state: string,
    cursor: unknown,
...
export function createCursorCodec(key: Uint8Array = randomBytes(32)): CursorCodec {
  const signingKey = Buffer.from(key)
  const sign = (payload: string): Buffer =>
    Buffer.from(createHmac('sha256', signingKey).update(payload, 'ascii').digest('base64url'), 'ascii')

  const encode = (scope: string, state: string, position: unknown): string => {
    const payload = Buffer.from(JSON.stringify({ v: 1, s: scope, t: state, p: position }), 'utf8').toString(
      'base64url',
    )
...
/** The default process-affine codec shared by embedded server instances. */
export const PROCESS_CURSOR_CODEC = createCursorCodec()
